(() => {
  ...
  function renderClassAbilities(body) {
    ...
    body.innerHTML = `<button type="button" class="quick-build-change" data-reset-class>← 重新選擇職業</button><div class="quick-build-ability-heading"><h3>${CLASS_LABELS[key]}${draft.choices.classOptions.classType ? `（${CLASS_TYPE_OPTIONS[key]?.find(option => option.id === draft.choices.classOptions.classType)?.label || ""}）` : ""}：屬性與背景加值</h3><div class="quick-build-ability-status"><span class="quick-build-status-pill${spent === 27 ? " is-complete" : ""}">購點 <strong>${spent}</strong> / 27</span><span class="quick-build-status-pill${bonusTotal === 3 ? " is-complete" : ""}">背景加值 <strong>${bonusTotal}</strong> / 3</span></div></div>${backgroundBonusWarning}<p class="quick-build-lead">使用 ▲／▼ 調整起始屬性與背景加值；起始屬性必須剛好分配 27 點，背景加值必須分配 3 點且單項最多 +2。</p><div class="quick-build-ability-help"><span>目前背景：<strong>${escapeHtml(BACKGROUND_LABELS[draft.choices.background] || "未選擇")}</strong></span><span>可加值：<strong>${escapeHtml((draft.choices.backgroundAbilities || []).join("、") || "無")}</strong></span></div><div class="quick-build-ability-actions"><button type="button" class="quick-build-ability-reset" data-reset-abilities>重設</button><button type="button" class="quick-build-ability-confirm" data-class-ability-confirm${classAbilitiesComplete() ? "" : " disabled"}>屬性確認</button></div><section class="quick-build-choice-panel quick-build-ability-grid"><div class="quick-build-ability-table-head"><span>屬性</span><span>基礎值</span><span>背景加值</span><span>總值</span></div>${ABILITY_ORDER.map(ability => {
