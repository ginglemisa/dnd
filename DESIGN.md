...
桌邊法師護甲沿用專注卡片的外觀，標示為獨立「持續效果」，於總覽與法術分頁提供解除操作。生效時僅桌邊 AC 數字使用 `styles.css` 的 `--warning`（依經典／暖紙及明暗切換），旁邊「(法護)」繼承原 AC 字色、字級；一般角色卡維持正常 AC。
...
認識角卡第 2 → 3 步先在高亮區域左下方顯示第 3 步提示（碰到螢幕邊界時內縮），提示框可拖曳，切換高亮時保留拖曳後的位置，拖曳本身不會推進導覽；選單與「決定屬性」高亮各等待玩家點擊畫面或提示按鈕才前進，保留鍵盤按鈕操作。高亮移動使用 600ms 緩動，減少動態效果時直接切換，停留不計時。在工具選單內框選按鈕前，先等待選單開啟動畫並將目標捲入可視範圍；窄螢幕或文案增長時，不依賴選單先前的捲動位置。

主題、技能版面與 PDF 欄位驗證使用 `node validate-ui-themes.js`；其他適用檢查見 [README.md](README.md#維護與驗證)。

樹膚術與大步奔行沿用同一持續效果卡片及警示色，分別標示「(樹膚)」與「(大步)」。樹膚術將桌邊 AC 下限設為 17，可與法師護甲並存；大步奔行為目前形態速度增加 10 呎。同一法術重施不累加，個別解除不影響其他效果；一般角色卡維持原數值。
